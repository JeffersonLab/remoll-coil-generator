...
# septapus shield
dx_septapus=6.0
dy_septapus=6.0  #18 24  Widening the septapus shield azimuthally
r_start_septapus= 35.0
r_end_septapus= 46.0  # 45 Lifting the shield up by a mm
...
for j in range(1,4):
  ...
  for i in ["C", "outer_E","inner_E"]: 
    ...
    out+="\n\t\t<section zOrder=\"1\" zPosition=\""+str(-yoff[i+str(j)]-p["C"+str(j)+"_dy"]/2)+"\" xOffset=\"0\" yOffset=\"0\" scalingFactor=\"0\"/>"
    out+="\n\t\t<section zOrder=\"2\" zPosition=\""+str(yoff[i+str(j)]+p["C"+str(j)+"_dy"]/2)+"\" xOffset=\"0\" yOffset=\"0\" scalingFactor=\"0\"/>"
    out+="\n\t</xtru>"
...
for j in ["mid","tb"]:
  ...
  for i in ["C4_", "outer_E4_","inner_E4_"]:
    ...
    out+="\n\t\t<section zOrder=\"1\" zPosition=\""+str(-yoff[i+str(j)]-p["C4_"+str(j)+"_dy"]/2)+"\" xOffset=\"0\" yOffset=\"0\" scalingFactor=\"0\"/>"
    out+="\n\t\t<section zOrder=\"2\" zPosition=\""+str(yoff[i+str(j)]+p["C4_"+str(j)+"_dy"]/2)+"\" xOffset=\"0\" yOffset=\"0\" scalingFactor=\"0\"/>"
    out+="\n\t</xtru>"
